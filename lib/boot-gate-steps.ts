...
export const BOOT_GATE_STEPS: BootStep[] = [
  ...
  },
  {
    id: "uptime_widget",
    label: "Widget uptime",
    description: "Prepara il widget di stato/uptime dei servizi.",
    originalOrder: 20,
    kind: "provider",
    blocksBoot: false,
    module: "lib/uptime-widget-context.tsx → UptimeWidgetProvider",
    reads: "AsyncStorage (uptime_widget_enabled)",
    writes: "context widget uptime",
    dependsOn: "floating_widget",
    dependedBy: "UptimeWidget",
    timeout: "nessuno",
    positionReason: "Provider UI overlay, accanto agli altri widget flottanti.",
    knownRisks: "Context value memoizzato con useMemo (verificato) → safe per il loop boot.",
  },
  {
    id: "auto_telemetry",
    label: "Telemetria automatica",
    description: "Avvia il rilevamento automatico della guida moto e la telemetria.",
    originalOrder: 21,
    kind: "provider",
    blocksBoot: false,
    module: "lib/auto-telemetry-context.tsx → AutoTelemetryProvider",
    reads: "prefs telemetria, calibrazione mount, sensori moto, auth",
    writes: "context auto-telemetry, sessioni telemetria",
    dependsOn: "uptime_widget",
    dependedBy: "TelemetryPanel, indicatori guida",
    timeout: "nessuno",
    positionReason: "Provider di funzionalità che dipende da auth/location, tenuto verso l'interno.",
    knownRisks: "Context value memoizzato con useMemo (verificato) → safe per il loop boot.",
  },
  {
    id: "gesture_handler",
