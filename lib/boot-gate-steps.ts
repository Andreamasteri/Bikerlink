...
export const BOOT_GATE_STEPS: BootStep[] = [
  ...
  },
  {
    ...
    kind: "provider",
    blocksBoot: true,
    module: "components/RootProviders.tsx → QueryPersistLayer (PersistQueryClientProvider)",
    reads: "AsyncStorage (@bikerlink/rq-cache-v1)",
    writes: "QueryClient idratato + persister",
    dependsOn: "language, online_focus_manager",
    dependedBy: "auth e ogni useQuery/useMutation",
    timeout: "persist throttle 3000ms; maxAge 24h",
    positionReason: "Sopra AuthProvider perché l'auth usa React Query per la sessione utente.",
    knownRisks: "Cache idratata corrotta o buster non allineato → dati stantii; gestito da buster 'v2'.",
  },
  {
    id: "auth",
  ...
  {
    id: "uptime_widget",
    label: "Widget uptime (admin)",
    description: "Prepara il pannellino di stato server per gli admin.",
    originalOrder: 19,
    kind: "provider",
    blocksBoot: false,
    module: "lib/uptime-widget-context.tsx → UptimeWidgetProvider",
    reads: "AsyncStorage (preferenza widget)",
    writes: "context uptime widget",
    dependsOn: "floating_widget",
    dependedBy: "AdminUptimeOverlay",
    timeout: "nessuno",
    positionReason: "Ultimo dei provider di context applicativi.",
    knownRisks: "—",
  },
  {
    id: "gesture_handler",
  ...
  {
    id: "keyboard",
    label: "Gestione tastiera",
    description: "Gestisce come la tastiera spinge i contenuti.",
    originalOrder: 21,
    kind: "provider",
    blocksBoot: false,
    module: "components/RootProviders.tsx → KeyboardProvider (react-native-keyboard-controller)",
    reads: "eventi tastiera nativi",
    writes: "context keyboard-controller",
    dependsOn: "gesture_handler",
    dependedBy: "form, chat",
    timeout: "nessuno",
    positionReason: "Dentro GestureHandlerRootView, sopra l'albero UI che usa la tastiera.",
    knownRisks: "Versione bloccata a 1.21.6: non aggiornare senza autorizzazione (crash noti).",
  },
  {
    id: "auto_telemetry",
    label: "Telemetria automatica",
    description: "Avvia la raccolta automatica dei dati di guida.",
    originalOrder: 22,
    kind: "provider",
    blocksBoot: false,
    module: "lib/auto-telemetry-context.tsx → AutoTelemetryProvider",
    reads: "TASK_TELEMETRY, GPS, AppState",
    writes: "context telemetria, upload per distanza",
    dependsOn: "keyboard, background_telemetry_task, location",
    dependedBy: "registrazione percorsi",
    timeout: "upload by distance (5km), non a timer",
    positionReason: "Innermost provider: avvolge direttamente i children dell'app.",
    knownRisks: "Macchina a stati con invariante 'una sola sorgente'; transizioni serializzate.",
  },
  {
    id: "ota_auto_update",
