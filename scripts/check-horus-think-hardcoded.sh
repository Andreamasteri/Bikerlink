...
# che chiamano Ollama, in modo da prevenire che un futuro script Horus
# ignori silenziosamente HORUS_THINK_ENABLED.
#
# Pattern approvato:
...
#   disabilitare il ragionamento via HORUS_THINK=0. Un futuro script che
#   hardcoda think:true ignora questo override e rende impossibile usare
#   la modalita' veloce (no-reasoning) in ambienti a bassa risorsa o nel
#   dry-run.
#
...
echo "Scanning scripts/ for hardcoded think:true..."
...
# Detects that the match position comes after a // comment opener on the same line
...
def is_in_comment(line: str, match_start: int, is_shell: bool) -> bool:
    ...
    # TypeScript/JS: whole-line comment (starts with //)
    if not is_shell and stripped.startswith("//"):
        return True

    # TypeScript/JS: inline comment — find the first // not inside a string
    # Simple heuristic: locate // before the match position
...
for root, dirs, files in os.walk(SCRIPTS_DIR):
    ...
    for fname in files:
        ...
        # Normalize to "scripts/..." without leading "./"
...
if [ "$FIRST_LINE" = "OK" ]; then
  echo "OK - No hardcoded think:true found in scripts/"
  echo ""
  echo "All Horus scripts use HORUS_THINK_ENABLED (or think:false)."
  exit 0
else
  echo ""
  ...
  echo ""
  echo "Suppression (only if think:true is intentional and documented):"
