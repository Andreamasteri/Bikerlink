...
if ! npx eas-cli@18 --version &>/dev/null 2>&1; then
  echo -e "  ${RED}✖${RESET}  eas-cli@18 non disponibile. Installare con: npm install -g eas-cli"
  exit 1
fi
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  EAS_OUTPUT=$(CI=1 npx eas-cli@18 build:view "$BUILD_ID" --json 2>/dev/null || echo '{"status":"ERROR_FETCHING"}')
