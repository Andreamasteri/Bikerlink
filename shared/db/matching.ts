...
export const bikerZavarrinaMatches = pgTable("biker_zavorrina_matches", {
  ...
  zavarrinaId: varchar("zavorrina_id", { length: 36 })
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  // Task #2680 — FK names auto-generated by Drizzle exceed 63 chars, PG
  // silently truncates them, and drizzle-kit always sees a diff. Using explicit
  // names that match what PostgreSQL already stores (63-char truncated values)
  // keeps the FK semantics identical while eliminating the perpetual mismatch.
  ...
  // Explicit FK names = exact 63-char PG-truncated versions of the auto-generated
  // names. Drizzle-kit compares by name: exact match → zero diff on every push.
  foreignKey({
  ...
  foreignKey({
    ...
    foreignColumns: [zavarrinaWishlistMotos.id],
  }).onDelete("cascade"),
]);
