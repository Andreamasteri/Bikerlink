...
- **Deploy build — niente Expo web export** (Task #1150, fix collaterale): `scripts/deploy-build.sh` non esegue più `npx expo export --platform web`. Il classificatore di deploy di Replit, dopo aprile 2026, riconosce la cartella `static-build/web/` (4.8 MB di JS bundle Expo) come segnale di "Expo web app statica" e rifiuta il deploy autoscale con il messaggio "build output is in 'static-build/web' while Autoscale deployments expect a server to run". Il build phase completa, ma il container non viene mai promosso live. La fix lascia 3 step (db:push → esbuild server → marker `static-build/index.html` minimale). Il server Express in produzione non ha bisogno del bundle web Expo: serve API, OTA endpoint, landing page (`server/templates/landing-page.html`) e pagine HTML statiche. Effetto collaterale accettato: la rotta `/web` (preview app in browser) restituisce 404 — riprenderemo il bundle web come artefatto separato se servirà.
