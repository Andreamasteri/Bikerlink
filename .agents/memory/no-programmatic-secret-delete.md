...
Confirmed via `typeof deleteSecrets === "undefined"` in the CodeExecution sandbox (only `deleteEnvVars` exists, and it only touches non-secret env vars). `requestSecrets` can overwrite a secret's value, but nothing lets the agent delete a secret key outright. <!-- pragma: allowlist secret -->
