...
**Nginx bikerlink fix (Jul 2026):** `/etc/nginx/sites-available/bikerlink` aveva
`listen 192.168.1.35:443 ssl` in tutti e 5 i server block — corretto a
`listen 192.168.0.100:443 ssl` via `sudo sed -i`. L'errore era mascherato dal
DuckDNS server block in `sites-enabled/graphhopper` (ora rimosso con il fix symlink).
