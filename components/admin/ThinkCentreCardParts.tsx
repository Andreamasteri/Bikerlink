...
/**
 * Blocco collassabile Valhalla — mostra stato (online/offline), latenza,
 * versione tile e profili costing disponibili.
 */
export function ValhallaBlock({
  ...
  return (
    <View style={styles.ghBlock}>
      ...
      >
        <MaterialCommunityIcons name="routes" size={18} color={statusColor} style={styles.ghHeaderIcon} />
        <View style={styles.ghHeaderText}>
          <Text style={styles.ghTitle}>Valhalla</Text>
          <Text style={styles.ghSubtitle}>
            {detail.configured
              ? detail.ok
        ...
        <View style={[styles.ghDot, { backgroundColor: statusColor }]} />
        <Ionicons name={open ? "chevron-up" : "chevron-down"} size={16} color={Colors.textSecondary} />
      </TouchableOpacity>

      {open && (
        <View style={styles.ghList}>
          <View style={styles.valhallaStatusRow}>
            <View style={[styles.ghDot, { backgroundColor: statusColor }]} />
            <Text style={styles.valhallaStatusLabel}>{statusLabel}</Text>
          </View>

          {detail.tileVersion && (
            <Text style={styles.ghAreaStatus}>Tile: {detail.tileVersion}</Text>
          )}

          {detail.ok && detail.activeProfiles.length > 0 && (
            <View style={styles.valhallaProfilesRow}>
          ...
          {detail.ok && detail.activeProfiles.length === 0 && (
            <Text style={styles.ghAreaStatus}>Nessun profilo rilevato</Text>
          )}
          ...
          {detail.configured && !fingerprint && (
            <Text style={styles.ghFingerprint}>token Replit: non configurato</Text>
          )}
...
/**
 * Blocco collassabile Nominatim — mostra stato servizio, latenza, versione
 * software e data ultimo aggiornamento OSM.
 */
export function NominatimBlock({
  ...
  return (
    <View style={styles.ghBlock}>
      ...
      >
        <MaterialCommunityIcons name="map-search-outline" size={18} color={statusColor} style={styles.ghHeaderIcon} />
        <View style={styles.ghHeaderText}>
          <Text style={styles.ghTitle}>Nominatim</Text>
          <Text style={styles.ghSubtitle}>
      ...
      {open && (
        <View style={styles.ghList}>
          ...
          {(detail.dbState || detail.geocodeLatencyMs != null || detail.softwareVersion || detail.dataUpdated) && (
            <View style={styles.nominatimMetaRow}>
              {detail.dbState && (
                <View style={[
                  styles.nominatimMetaChip,
                  detail.dbState === "error" && styles.nominatimMetaChipError,
                ]}>
                  <Ionicons
              ...
              {detail.geocodeLatencyMs != null && (
                <View style={styles.nominatimMetaChip}>
                  <Ionicons name="location-outline" size={10} color="#60a5fa" />
                  <Text style={styles.nominatimMetaText}>geocode {detail.geocodeLatencyMs} ms</Text>
                </View>
              )}
              {detail.softwareVersion && (
                <View style={styles.nominatimMetaChip}>
                  <Ionicons name="code-outline" size={10} color="#60a5fa" />
                  <Text style={styles.nominatimMetaText}>v{detail.softwareVersion}</Text>
                </View>
              )}
              {detail.dataUpdated && (
                <View style={styles.nominatimMetaChip}>
                  <Ionicons name="time-outline" size={10} color="#60a5fa" />
                  <Text style={styles.nominatimMetaText}>OSM {formatDataUpdated(detail.dataUpdated)}</Text>
                </View>
              )}
            </View>
          )}
          ...
          {!detail.configured && (
            <View style={styles.nominatimPublicNote}>
              <Ionicons name="information-circle-outline" size={11} color="#f59e0b" />
              <Text style={styles.nominatimPublicNoteText}>
...
const styles = StyleSheet.create({
  ...
  valhallaStatusRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  valhallaStatusLabel: { fontFamily: "Inter_400Regular", fontSize: 11, color: Colors.textSecondary },
  valhallaProfilesRow: {
    flexDirection: "row",
    flexWrap: "wrap",
    gap: 6,
  ...
  nominatimMetaRow: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 4 },
  nominatimMetaChip: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
  ...
  nominatimMetaChipError: {
    backgroundColor: "rgba(239, 68, 68, 0.08)",
    borderColor: "rgba(239, 68, 68, 0.25)",
  },
  nominatimMetaText: { fontFamily: "Inter_500Medium", fontSize: 10, color: "#60a5fa" },
  nominatimPublicNote: {
    flexDirection: "row",
