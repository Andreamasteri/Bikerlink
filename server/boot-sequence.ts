...
export async function runBootSequence(server: Server, errorHandlersReady: Promise<void>): Promise<void> {
  ...
  // ── DragonflyDB VPS TCP bridge (cloudflared access tcp) ───────────────────
  // Apre il path privato verso DragonflyDB sul VPS quando REDIS_TUNNEL_HOSTNAME
  // e le credenziali Cloudflare Access sono configurati. È best-effort e non
  // blocca il boot: Redis ricade in-memory finché il bridge non è disponibile.
  try {
...
async function runPostReady(needsFakeSeed: boolean): Promise<void> {
  ...
  // ── Post-deploy analysis: solo su richiesta manuale ─────────────────────────
  // L'analisi post-deploy NON viene avviata automaticamente al boot.
  // Va eseguita manualmente dall'operatore tramite il workflow "Post-Deploy Analysis"
  // oppure con: FORCE_RERUN=1 bash scripts/post-deploy-analysis.sh
  // Il deploy stamp (server_dist/.deploy-stamp) scritto da deploy-build.sh è
  // disponibile per consultazione, ma non innesca nessun processo automatico.
}
