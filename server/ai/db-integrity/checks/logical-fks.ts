...
async function logicalFk(label: string, q: string, sampleQ: string): Promise<CheckResult> {
  try {
    const cnt = await db.execute(sql.raw(q));
    const count = Number((cnt.rows?.[0] as { c?: number } | undefined)?.c ?? 0);
    if (!count) return { ok: true, count: 0, sample: [] };
    const smp = await db.execute(sql.raw(sampleQ));
...
const checks: IntegrityCheck[] = [
  {
    ...
    async query() {
      ...
      return logicalFk("match-vs-banned",
        `SELECT COUNT(*)::int AS c FROM biker_zavorrina_matches m JOIN users u ON u.id = m.zavorrina_id WHERE m.created_at > NOW() - INTERVAL '24 hours' AND u.shadow_banned_at IS NOT NULL AND u.shadow_banned_until IS NULL`,
        `SELECT m.id, m.zavorrina_id, u.shadow_banned_at FROM biker_zavorrina_matches m JOIN users u ON u.id = m.zavorrina_id WHERE m.created_at > NOW() - INTERVAL '24 hours' AND u.shadow_banned_at IS NOT NULL AND u.shadow_banned_until IS NULL LIMIT 10`,
      );
    },
  },
  {
    ...
    async query() {
      ...
      return logicalFk("reports-on-deleted",
        `SELECT COUNT(*)::int AS c FROM reports r JOIN users u ON u.id = r.reported_user_id WHERE r.status IN ('open','pending') AND u.status = 'deleted'`,
        `SELECT r.id, r.reported_user_id, u.status FROM reports r JOIN users u ON u.id = r.reported_user_id WHERE r.status IN ('open','pending') AND u.status = 'deleted' LIMIT 10`,
      );
    },
  },
  {
    ...
    async query() {
      ...
      return logicalFk("embeddings-deleted",
        `SELECT COUNT(*)::int AS c FROM embeddings e JOIN users u ON u.id = e.entity_id WHERE e.entity_type = 'user' AND u.status = 'deleted'`,
        `SELECT e.id, e.entity_id, u.status FROM embeddings e JOIN users u ON u.id = e.entity_id WHERE e.entity_type = 'user' AND u.status = 'deleted' LIMIT 10`,
      );
    },
  },
  {
    ...
    async query() {
      ...
      return logicalFk("self-match",
        `SELECT COUNT(*)::int AS c FROM biker_biker_matches WHERE biker1_id = biker2_id`,
        `SELECT id, biker1_id, biker2_id FROM biker_biker_matches WHERE biker1_id = biker2_id LIMIT 10`,
      );
    },
  },
];
