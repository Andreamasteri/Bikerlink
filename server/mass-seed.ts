...
import {
  ...
  randOffset, randBirthYear, pickRandom, getMotoYear, getBio, getWelcomeMessage,
  distributeUniformly,
} from "./mass-seed-data";
...
async function ensureOfficialAccount(): Promise<string> {
  ...
  const hashedPw = await bcrypt.hash("BikerLinkSystem2024!", 12);
  const user = await storage.createUser({
    nickname: "BikerLink_Official",
    email: "system@bikerlink.it",
    password: hashedPw,
    userType: "biker",
    sex: "M",
    role: "admin",
    ...
    for (let batchStart = 0; batchStart < specs.length; batchStart += BATCH_SIZE) {
      ...
      for (const spec of batch) {
        try {
          ...
          const bio = getBio(spec.userType, spec.sex);
          ...
          if (spec.userType === "biker" || spec.userType === "coppia") {
            const moto = pickRandom(MOTORCYCLES);
          ...
          if (spec.userType === "zavorrina") {
            const wishlist = await storage.createOrUpdateWishlist(newUser.id, "Cerco un biker per bei giri in moto");
            const desiredMoto = pickRandom(MOTORCYCLES);
            await db.insert(zavarrinaWishlistMotos).values({
              wishlistId: wishlist.id,
              brand: desiredMoto.brand,
              model: desiredMoto.model,
              motorcycleType: desiredMoto.type,
              ridingStyle: desiredMoto.style,
            });
          }

          const welcomeMsg = getWelcomeMessage(spec.userType, spec.sex);
          const [conv] = await db.insert(conversations).values({
            conversationType: "private",
          }).returning();
          await db.insert(conversationParticipants).values([
            { conversationId: conv.id, userId: officialId },
            { conversationId: conv.id, userId: newUser.id },
          ]);
          ...
          massSeedStatus.created++;
        } catch (err: any) {
          if (err?.code === "23505") {
            continue;
          }
          console.error(`Error creating fake user:`, err?.message);
        }
      }

      if (batchStart % (BATCH_SIZE * 5) === 0 && batchStart > 0) {
