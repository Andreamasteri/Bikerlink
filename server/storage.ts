...
export class DatabaseStorage implements IStorage {
  ...
  async countAvailableBikers(_since: Date, countries?: string[]): Promise<number> {
  ...
  async countAvailableZavorrine(_since: Date, countries?: string[]): Promise<number> {
    const conditions: any[] = [
      eq(users.status, "active"),
      eq(userProfiles.isAvailable, true),
  ...
  async getAvailableBikersList(_since: Date, lat?: number, lng?: number, countries?: string[]): Promise<any[]> {
    const distanceExpr = lat != null && lng != null
      ? sql<number>`(6371 * acos(cos(radians(${lat})) * cos(radians(${userProfiles.latitude})) * cos(radians(${userProfiles.longitude}) - radians(${lng})) + sin(radians(${lat})) * sin(radians(${userProfiles.latitude}))))`.as("distance")
      : sql<number>`0`.as("distance");
    const conditions: any[] = [
  ...
  async getAvailableZavorrinaList(_since: Date, lat?: number, lng?: number, countries?: string[]): Promise<any[]> {
    const distanceExpr = lat != null && lng != null
      ? sql<number>`(6371 * acos(cos(radians(${lat})) * cos(radians(${userProfiles.latitude})) * cos(radians(${userProfiles.longitude}) - radians(${lng})) + sin(radians(${lat})) * sin(radians(${userProfiles.latitude}))))`.as("distance")
      : sql<number>`0`.as("distance");
    const conditions: any[] = [
