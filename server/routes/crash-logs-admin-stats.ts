...
import {
  requireAdmin,
  DERIVED_TYPE_EXPR,
  SIGNAL_TYPES_DIAGNOSTIC,
  SIGNAL_TYPES_CONTEXT,
} from "./crash-logs";

export const adminStatsRouter = Router();

adminStatsRouter.get("/stats", requireAdmin, (req: Request, res: Response): void => {
  const {
    ...
    .then(([typeRows, versionRows, trendRows, rateRows, ramRows]) => {
      ...
      res.json({
        ...
        byVersion: versionRows.rows as {
          version: string; crash_system: number; crash_js: number;
          restart_loop: number; js_thread_freeze: number; gps_flood: number;
          memory_pressure: number; native_module_missing: number; total: number;
        }[],
        dailyTrend: trendRows.rows as {
          day: string; crash_system: number; crash_js: number;
          restart_loop: number; js_thread_freeze: number; gps_flood: number;
          memory_pressure: number; native_module_missing: number;
        }[],
        crashFreeRate24h,
        ramMedianCrashMb,
      });
...
adminStatsRouter.get("/restart-loop-summary", requireAdmin, (req: Request, res: Response): void => {
  db.execute(sql`
    SELECT
...
adminStatsRouter.get("/signal-frequency", requireAdmin, (req: Request, res: Response): void => {
...
adminStatsRouter.get("/alerts", requireAdmin, (req: Request, res: Response): void => {
    ...
    .then((result) => {
      res.json({
        alerts: result.rows as {
          ...
          js_thread_freeze: number; gps_flood: number;
          memory_pressure: number; native_module_missing: number;
        }[],
        threshold,
      });
