/**
 * Guard: app_store_url persists across server restarts.
 *
 * Two categories of checks:
 *
 * 1. ROUTE LAYER — GET /app-store-url and PUT /app-store-url
 ...
 *    upsertAppSetting (or any raw SQL) with the key "app_store_url".
...
describe("app_store_url — route persistence", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("GET returns empty string when no value has been saved yet", async () => {
    ...
    const app = buildApp();
    const res = await request(app).get("/api/admin/settings/app-store-url");

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ url: "" });
    expect(mockGetAppSetting).toHaveBeenCalledWith("app_store_url");
  });
  ...
  it("PUT calls upsertAppSetting with the correct key and URL", async () => {
    const testUrl = "https://apps.apple.com/app/bikerlink/id123456789";
    mockUpsertAppSetting.mockResolvedValueOnce({ key: "app_store_url", value: testUrl });

    const app = buildApp();
    const res = await request(app)
      .put("/api/admin/settings/app-store-url")
      .send({ url: testUrl });

    expect(res.status).toBe(200);
    expect(mockUpsertAppSetting).toHaveBeenCalledWith("app_store_url", testUrl);
  });
  ...
  it("GET after PUT returns the saved URL (simulates restart read-back)", async () => {
    ...
    mockGetAppSetting.mockResolvedValueOnce({ key: "app_store_url", value: testUrl });
  ...
  it("PUT with an arbitrary string value saves it (schema accepts any string)", async () => {
    ...
    mockUpsertAppSetting.mockResolvedValueOnce({ key: "app_store_url", value: rawValue });
    ...
    expect(res.status).toBe(200);
    expect(mockUpsertAppSetting).toHaveBeenCalledWith("app_store_url", rawValue);
  });

  it("PUT with missing url field saves undefined (schema field is optional)", async () => {
    mockUpsertAppSetting.mockResolvedValueOnce({ key: "app_store_url", value: undefined });
    ...
    expect(res.status).toBe(200);
    expect(mockUpsertAppSetting).toHaveBeenCalledWith("app_store_url", undefined);
  });
  ...
  it("PUT returns 500 when storage throws", async () => {
    ...
    const res = await request(app)
      .put("/api/admin/settings/app-store-url")
      .send({ url: "https://apps.apple.com/app/bikerlink/id123456789" });
...
// ---------------------------------------------------------------------------
// Static boot guard — no boot file may write app_store_url
// ---------------------------------------------------------------------------

describe("app_store_url — boot-phase static guard", () => {
  const KEY = "app_store_url";
  ...
  it("no boot-phase file in server/ upserts or resets app_store_url", () => {
