...
// Task #591: Quebracho removed — only Horus reachability via isOllamaReachable.
// The job-gate mock is no longer needed for isQuebrachoUnreachable.
...
describe("getCoordinatorSnapshot", () => {
  ...
  // Task #591: Quebracho removed — snapshot no longer exposes quebrachoReachable/directives.quebracho.
});

describe("direttive multi-issuer — admin_manual > horus (Task #591: Quebracho rimosso)", () => {
  it("pausa Horus blocca il ciclo, resume la sblocca", async () => {
    ...
    const snapshot2 = await getCoordinatorSnapshot();
    expect(snapshot2.directives.horus).toBeNull();
    expect((await canRunCycleNow()).allowed).toBe(true);
  });

  it("una pausa Horus viene ignorata (fallback deterministico) se Horus è irraggiungibile", async () => {
    await applyCoordinatorDirective("pause", { reason: "pausa horus" }, "horus");
    isOllamaReachableMock.mockResolvedValue(false);
  ...
  it("admin_manual resta efficace anche con pausa horus attiva e Horus irraggiungibile", async () => {
    await applyCoordinatorDirective("pause", { reason: "pausa horus" }, "horus");
