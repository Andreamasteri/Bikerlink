/**
 * HTTP-level regression tests — Fake Position Pipeline
 *
 *  BUG 1 — PUT /me: applyFakeZones must run before persisting lat/lng
 *  BUG 2 — captureFirstAvailabilityLocation must receive fLat/fLng, not raw coords
 *  BUG 3 — POST /app-close: must honour global offline_position_randomize_default setting
 *
 * Pure unit tests for applyFakeZones are in fake-zones-regression.test.ts
 */
