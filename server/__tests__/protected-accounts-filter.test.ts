...
describe("MatchingStorage.getCompatibleWishlistGaragePairs — protected account exclusion", () => {
  ...
  it("excludes the BikerLink_Official account (biker role) and keeps real users", async () => {
    ...
    expect(bikerIds).not.toContain(PROTECTED_USER_ID);   // protected absent
    expect(bikerIds).toContain("user-real-biker");        // control present
  });
  ...
  it("negative path: without protection filter, BikerLink_Official WOULD appear", async () => {
    // Simulate absent SQL clause: db.execute returns ALL seeded rows unfiltered
...
describe("ProposalsStorage.getActiveProposalCandidatePairs — protected account exclusion", () => {
  it("excludes proposals belonging to BikerLink_Official and keeps real proposals", async () => {
    ...
    expect(allIds).not.toContain("proposal-C");   // BikerLink_Official's proposal — absent
    expect(allIds).toContain("proposal-A");        // RealUser1's proposal — present
    expect(allIds).toContain("proposal-B");        // RealUser2's proposal — present
  });
  ...
  it("returns exactly the non-protected pairs (selective exclusion, not empty result)", async () => {
    ...
    // 2 pairs seeded (1 real + 1 protected-involved) → only 1 should survive
    expect(pairs).toHaveLength(1);
  ...
  it("negative path: without protection filter, BikerLink_Official proposal WOULD appear", async () => {
    // No filter: mock returns all rows including the protected pair
    ...
    const allIds = pairs.flatMap((p) => [p.id1, p.id2]);
    expect(allIds).toContain("proposal-C");   // protected pair present without filter
  });
...
describe("GET /routes/compatible-bikers/:id — protected account exclusion", () => {
  it("excludes BikerLink_Official and returns only real nearby bikers", async () => {
    ...
    expect(nicknames).not.toContain(PROTECTED_NICKNAME);   // protected absent
    expect(nicknames).toContain("RealBiker1");              // control present
  });
  ...
  it("returns a non-empty biker list (selective exclusion, not empty result)", async () => {
    ...
    expect(res.status).toBe(200);
    // 2 rows seeded (1 real + 1 protected) → only 1 should survive
  ...
  it("negative path: without protection filter, BikerLink_Official WOULD appear", async () => {
    // No filter: return all rows including protected
    ...
    expect(res.status).toBe(200);
    const nicknames = (res.body.bikers as Array<{ nickname: string }>).map((b) => b.nickname);
    expect(nicknames).toContain(PROTECTED_NICKNAME);   // appears without filter
    expect(nicknames).toContain("RealBiker1");          // control also present
  });
