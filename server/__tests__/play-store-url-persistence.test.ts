/**
 * Guard: play_store_url persists across server restarts.
 *
 * Two categories of checks:
 *
 * 1. ROUTE LAYER — GET /play-store-url and PUT /play-store-url
 ...
 *    upsertAppSetting (or any raw SQL) with the key "play_store_url".
...
describe("play_store_url — route persistence", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("GET returns empty string when no value has been saved yet", async () => {
    ...
    const app = buildApp();
    const res = await request(app).get("/api/admin/settings/play-store-url");

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ url: "" });
    expect(mockGetAppSetting).toHaveBeenCalledWith("play_store_url");
  });
  ...
  it("PUT calls upsertAppSetting with the correct key and URL", async () => {
    const testUrl = "https://play.google.com/store/apps/details?id=com.bikerlink";
    mockUpsertAppSetting.mockResolvedValueOnce({ key: "play_store_url", value: testUrl });
    ...
    expect(res.status).toBe(200);
    expect(mockUpsertAppSetting).toHaveBeenCalledWith("play_store_url", testUrl);
  });
  ...
  it("GET after PUT returns the saved URL (simulates restart read-back)", async () => {
    ...
    mockGetAppSetting.mockResolvedValueOnce({ key: "play_store_url", value: testUrl });
  ...
  it("PUT with an arbitrary string value saves it (schema accepts any string)", async () => {
    ...
    mockUpsertAppSetting.mockResolvedValueOnce({ key: "play_store_url", value: rawValue });
    ...
    expect(res.status).toBe(200);
    expect(mockUpsertAppSetting).toHaveBeenCalledWith("play_store_url", rawValue);
  });

  it("PUT with missing url field saves undefined (schema field is optional)", async () => {
    mockUpsertAppSetting.mockResolvedValueOnce({ key: "play_store_url", value: undefined });
    ...
    expect(res.status).toBe(200);
    expect(mockUpsertAppSetting).toHaveBeenCalledWith("play_store_url", undefined);
  });
  ...
  it("PUT returns 500 when storage throws", async () => {
    mockUpsertAppSetting.mockRejectedValueOnce(new Error("DB error"));

    const app = buildApp();
    const res = await request(app)
      .put("/api/admin/settings/play-store-url")
      .send({ url: "https://play.google.com/store/apps/details?id=com.bikerlink" });
...
// ---------------------------------------------------------------------------
// Static boot guard — no boot file may write play_store_url
// ---------------------------------------------------------------------------

describe("play_store_url — boot-phase static guard", () => {
  const KEY = "play_store_url";
  ...
  it("no boot-phase file in server/ upserts or resets play_store_url", () => {
