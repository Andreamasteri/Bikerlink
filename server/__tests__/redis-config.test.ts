...
afterEach(() => {
  ...
  delete process.env.REDIS_TUNNEL_HOSTNAME;
  delete process.env.REDIS_TUNNEL_LOCAL_PORT;
});
...
describe("Redis provider selection", () => {
  ...
  it("routes REDIS_URL through the local Access TCP bridge when configured", () => {
    process.env.REDIS_URL = "rediss://:vps-secret@dragonfly-akuma.biker-link.net:6379";
    process.env.REDIS_TUNNEL_HOSTNAME = "dragonfly-akuma.biker-link.net";
    process.env.REDIS_TUNNEL_LOCAL_PORT = "16379";
