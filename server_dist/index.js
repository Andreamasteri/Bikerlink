    ...
    DatabaseStorage = class {
      ...
      async getNearbyUsers(lat, lng, radiusKm, countries) {
        const conditions = [
          ...
          (0, import_drizzle_orm2.notInArray)(users.role, ["admin", "moderatore"]),
      ...
      async getOnlineUsersList(since, lat, lng, countries) {
        ...
        const conditions = [(0, import_drizzle_orm2.eq)(users.status, "active"), (0, import_drizzle_orm2.gte)(users.lastLoginAt, since), (0, import_drizzle_orm2.eq)(users.ghostMode, false), (0, import_drizzle_orm2.notInArray)(users.role, ["admin", "moderatore"])];
        if (countries && countries.length > 0) {
...
router3.get("/online-list", requireAuth2, async (req, res) => {
  try {
    ...
    if (includeOffline) {
      ...
      const offlineConds = [eq11(usersTable.status, "active"), or4(lt(usersTable.lastLoginAt, fifteenMinutesAgo), isNull2(usersTable.lastLoginAt)), eq11(usersTable.ghostMode, false), notInArr(usersTable.role, ["admin", "moderatore"])];
